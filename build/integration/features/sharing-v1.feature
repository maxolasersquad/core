Feature: sharing
  ...
  Scenario: Correct webdav sharePermissions for owned file
    Given user "user0" exists
    And User "user0" uploads file with content "foo" to "/tmp.txt"
    When as "user0" gets properties of folder "/tmp.txt" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "19"

  Scenario: Correct webdav sharePermissions for received file with edit and reshare permissions
    Given user "user0" exists
    And user "user1" exists
    And User "user0" uploads file with content "foo" to "/tmp.txt"
  ...
  Scenario: Correct webdav sharePermissions for received file with edit permissions but no reshare permissions
    Given user "user0" exists
    And user "user1" exists
    And User "user0" uploads file with content "foo" to "/tmp.txt"
    And file "tmp.txt" of user "user0" is shared with user "user1"
    ...
    When as "user1" gets properties of folder "/tmp.txt" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "0"

  Scenario: Correct webdav sharePermissions for received file with reshare permissions but no edit permissions
    Given user "user0" exists
    And user "user1" exists
    And User "user0" uploads file with content "foo" to "/tmp.txt"
    And file "tmp.txt" of user "user0" is shared with user "user1"
    ...
    When as "user1" gets properties of folder "/tmp.txt" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "17"

  Scenario: Correct webdav sharePermissions for owned folder
    Given user "user0" exists
    And user "user0" created a folder "/tmp"
    When as "user0" gets properties of folder "/" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "31"

  Scenario: Correct webdav sharePermissions for received folder with all permissions
    Given user "user0" exists
    And user "user1" exists
    And user "user0" created a folder "/tmp"
    And file "/tmp" of user "user0" is shared with user "user1"
  ...
  Scenario: Correct webdav sharePermissions for received folder with all permissions but edit
    Given user "user0" exists
    And user "user1" exists
    And user "user0" created a folder "/tmp"
    And file "/tmp" of user "user0" is shared with user "user1"
    ...
    When as "user1" gets properties of folder "/tmp" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "29"

  Scenario: Correct webdav sharePermissions for received folder with all permissions but create
    Given user "user0" exists
    And user "user1" exists
    And user "user0" created a folder "/tmp"
    And file "/tmp" of user "user0" is shared with user "user1"
    ...
    When as "user1" gets properties of folder "/tmp" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "27"

  Scenario: Correct webdav sharePermissions for received folder with all permissions but delete
    Given user "user0" exists
    And user "user1" exists
    And user "user0" created a folder "/tmp"
    And file "/tmp" of user "user0" is shared with user "user1"
    ...
    When as "user1" gets properties of folder "/tmp" with
      |{http://owncloud.org/ns}sharePermissions|
    Then the single response should contain a property "{http://owncloud.org/ns}sharePermissions" with value "23"

  Scenario: Correct webdav sharePermissions for received folder with all permissions but share
    Given user "user0" exists
    And user "user1" exists
    And user "user0" created a folder "/tmp"
    And file "/tmp" of user "user0" is shared with user "user1"
